...
- WORK MATHS PRIVATELY BEFORE WRITING. Derive the answer fully in your head before emitting anything for that question. The worked solution must contain ONLY clean final reasoning. NEVER write any of: "wait", "hmm", "let me redo", "let me recheck", "let me re-derive", "let's recheck", "let's be precise", "let's redo cleanly", "actually", "correction:", "correction note:", "Answer (corrected):", "Answer (confirmed):". If your first attempt is wrong, silently re-derive on scratch and emit ONLY the corrected version — the student must never see the false start, the recheck, or the re-derivation.
- NEVER write meta-commentary aimed at the parent/student/tutor inside the worksheet: no "Note to parent/student", "Note for parent/student", "please disregard", "please contact your tutor", "the question above contains an error", "Questions X and Y above contain errors". If you cannot produce a clean question, REPLACE it with a different question on the same topic — do not ship the broken one with an apology.
