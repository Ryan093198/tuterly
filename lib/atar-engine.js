...
import { isEnglishSubject } from "./vce-subjects";
 ...
 *   ignored: { subject: string, score: number }[],
 * }}
 */
 ...
 * }}
 */
export function calculateAtar(subjects) {
  const { english, primary4, increment5, increment6 } = categoriseSubjects(subjects);
 ...
 */
export function suggestImprovements(subjects, targetAtar, { limit = 4 } = {}) {
  const baseResult = calculateAtar(subjects);
  if (baseResult.atar >= targetAtar) return [];

  // Try +5 on each Primary-4 subject one at a time, see the impact
  const suggestions = [];
  for (const ps of baseResult.primary4) {
    if (ps.score >= 50) continue;
    const bumped = subjects.map((s) =>
      s.subject === ps.subject ? { ...s, score: Math.min(50, s.score + 5) } : s
    );
    const bumpedResult = calculateAtar(bumped);
    suggestions.push({
      subject: ps.subject,
      currentScore: ps.score,
      targetScore: Math.min(50, ps.score + 5),
  ...
  // And increments (10% weight) - smaller impact but worth noting
  for (const ps of [baseResult.increment5, baseResult.increment6]) {
    if (!ps || ps.score >= 50) continue;
    const bumped = subjects.map((s) =>
      s.subject === ps.subject ? { ...s, score: Math.min(50, s.score + 5) } : s
    );
    const bumpedResult = calculateAtar(bumped);
