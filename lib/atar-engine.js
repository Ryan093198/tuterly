...
export function findSimilarCourses(allCourses, studentAtar, subjects, options = {}) {
  const { excludeId = null, limit = 8, selectedCategory = null } = options;
  const scored = allCourses
    ...
    .map((c) => {
      ...
      const unmetPrereqs = prereqResults
        .filter((r) => !r.met)
        .map((r) => ({ subject: r.subject, minimumScore: r.requiredScore }));
      const atarGap = studentAtar - c.guaranteedAtar;
      const sameCategory = selectedCategory ? c.category === selectedCategory : false;
      return { course: c, prereqResults, allPrereqsMet, unmetPrereqs, atarGap, sameCategory };
    })
    .filter((entry) => entry.atarGap >= -2);

  // When a category is picked, prioritise same-category matches first.
  // Fall back to other categories only if we don't have enough same-category hits.
  const sorter = (a, b) => {
  ...
  if (selectedCategory) {
    const sameCat = scored.filter((e) => e.sameCategory).sort(sorter);
    if (sameCat.length >= limit) return sameCat.slice(0, limit);
    const otherCat = scored.filter((e) => !e.sameCategory).sort(sorter);
    return [...sameCat, ...otherCat].slice(0, limit);
  }

  return scored.sort(sorter).slice(0, limit);
}
