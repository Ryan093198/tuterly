...
// but with a fixed 6-question / 2-Foundation / 2-Standard / 2-Extension
...
(two Extension questions: 5-6)

WORKSHEET RULES:
- Output EXACTLY 6 questions in this split: questions 1-2 Foundation, 3-4 Standard, 5-6 Extension. The "## Foundation" / "## Standard" / "## Extension" headers MUST appear above each block.
- With only two questions per band there is no room for a weak one. Each pair must cover DIFFERENT sub-skills within the band, not the same question with new numbers, so the six together still survey the whole topic.
...
export function buildWorksheetUserMessage(args) {
  ...
  return [
    ...
    topicDescription
      ...
      : null,
    "",
    "Generate the 6-question worksheet now (2 Foundation, 2 Standard, 2 Extension).",
    variantSeed
