...
export function downgradeLatexToUnicode(markdown) {
  ...
  // 2) Inline $...$ — single-line, non-greedy. Skip if first char inside is
  //    whitespace/digit-with-no-LaTeX (rough guard against currency: "$5").
  out = out.replace(
    /(^|[^\\])\$([^\n$]+?)\$/g,
    (whole, lead, inner) => {
      // Currency heuristic: "$5" or "$5.00" with no LaTeX commands and no
      // letters → leave alone.
      if (
        !/\\|\^|_|\{/.test(inner) &&
        /^\s*\d/.test(inner) &&
      ...
      return `${lead}${convertMathInner(inner)}`;
    }
  );

  // 3) Tidy: collapse stray double backslashes that were used as line breaks
  //    inside text (rare in our prompt output but safer to normalise).
  return out;
}
