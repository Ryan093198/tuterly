    ...
    if (ch === "$" && md[i - 1] !== "\\") {
      ...
      if (j < n && md[j] === "$") {
        ...
        if (looksLikeProse(content)) {
          ...
          // Legitimate math span — pass through verbatim AND skip past the
          // close, so the next `$` after it isn't mistaken for a new opener
          // that pairs with the following math span's opener (which would
          // wrap the prose between them and escape it).
          out.push(md.slice(i, j + 1));
        }
        i = j + 1;
        continue;
      }
      // No matching close on this line — let the markdown renderer handle it.
    }
