...
function cleanTopic(raw) {
  let t = raw.trim();
  // Remove VCAA codes like "VC2M8P03", "VC2M10A04"
  t = t.replace(/\s*[—\-–&,]\s*VC2M\w+/g, "").trim();
  t = t.replace(/VC2M\w+\s*[—\-–&:,]?\s*/g, "").trim();
  // Remove strand prefixes
  t = t
    .replace(
      /^(Probability|Algebra|Number|Measurement|Space|Statistics|Geometry|Calculus|Data Analysis)\s*[—\-–:]\s*/i,
      ""
    )
    .trim();
...
export function detectOverallTopic(reportText) {
  if (!reportText) return "";
