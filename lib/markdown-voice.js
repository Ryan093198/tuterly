...
export function stripEmDashes(md) {
  ...
  while (i < n) {
    ...
    // Em dash (\u2014) or en dash (\u2013) in prose: the AI tell. Replace with
    // comma + space, collapsing surrounding spaces so we don't double them.
    // Matched by unicode escape, NOT a literal dash char, and NOT the ASCII
    // hyphen "-" (0x2D) - matching the hyphen was the bug that turned "---"
    // rules into ",,," and "- " bullets into ", ".
    if (ch === "\u2014" || ch === "\u2013") {
...
// Programmatic enforcement of the report's "no URLs / no links" rule
// (audit H3). The prompt already forbids links, but a prompt-injection payload
// in session notes or an uploaded resource could push the model to emit one.
// This is the belt-and-braces server-side check applied before the report is
// saved: markdown links collapse to their visible text, and any bare URL is
// replaced with a neutral placeholder. Report content is prose + KaTeX, never
// legitimate hyperlinks, so this is safe to run unconditionally.
export function stripLinks(md) {
  if (!md || typeof md !== "string") return md;
  let out = md;

  // [visible text](http…) or [text](www…)  ->  visible text
  out = out.replace(
    /\[([^\]]*)\]\(\s*(?:https?:\/\/|www\.)[^)]*\)/gi,
    "$1"
  );

  // Autolinks <http…> -> stripped
  out = out.replace(/<\s*(?:https?:\/\/|www\.)[^>]*>/gi, "");

  // Bare URLs left in prose -> neutral placeholder (keeps the sentence intact).
